...
function initClient($rc)
{
  for ($ix=0; $ix < $rc; $ix++) {
    $client[$ix] = new PlutonClient("leak");
  }
}
...
$stInternal = memory_get_usage(false);
$stExternal = memory_get_usage(true);
...
if (($diff > 100) || ($diff < 0)) {
  print "Scotty - we appear to have a client leak according to ps!\n";
